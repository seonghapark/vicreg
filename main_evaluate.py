...
from dataset_main_evaluate import BasicDataset
...
def get_arguments():
    ...
    # Data
    parser.add_argument("--data-dir1", type=str, help="path to dataset")
    parser.add_argument("--data-dir2", type=str, help="path to dataset")
...
class Main():
    ...
    def run(self, args):
        ...
        for step, (x, y) in enumerate(zip(train_loader, val_loader), len(val_loader)):
            ...
            val.append((args.sim_coeff * repr_loss * 100).item())
...
if __name__ == "__main__":
    ...
    if args.single_folder:
        val = np.asarray(main.run(args))
        print(np.mean(val))
    else:
        name = args.data_dir1
        folders = sorted(os.listdir(args.data_dir1))
        ...
        for i in folders:
            ...
            if ii > 100:
               pass
                ...
                target = name + i + '/'
                print('TARGET', target)
                args.data_dir1 = target
                val = np.asarray(main.run(args))
                print(np.mean(val), gc.collect())
